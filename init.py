...
def run():
    ...
    if(kasutajanimi == "" or salasona == ""):
        xbmcgui.Dialog().notification("IptvEIks Privado Addon", "Configura el plugin.")
        xbmcaddon.Addon().openSettings()
    pnimi = get_live("WHRyZWFtLUNvZGVzIA==") #summary addon.xml
    ...
    if get_live("S09ESQ==") not in open(addonDir+"/"+sync_data("YWRkb24ueG1s")).read():
       check_user() #Debe coincidir con "KODI" en la descripcion de addon.xml
...
def peamenyy(params):
    ...
    if channels == 1:
       ...
       plugintools.add_item( action=vod_channels("ZXhlY3V0ZV9haW5mbw=="),   title=vod_channels("TWkgY3VlbnRh") , thumbnail="" , fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) , folder=True )
       plugintools.add_item( action=vod_channels("c2VjdXJpdHlfY2hlY2s="),  title=vod_channels("VFY=") , thumbnail="" , fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) , folder=True )
       plugintools.add_item( action=vod_channels("ZGV0ZWN0X21vZGlmaWNhdGlvbg=="),   title=vod_channels("Vk9E") , thumbnail="" , fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) , folder=True )
       plugintools.add_item( action=vod_channels("bGljZW5zZV9jaGVjaw=="), title=vod_channels("Q29uZmlndXJhY2nDs24=") , thumbnail="" , fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=") ), folder=False )
...
def stream_video(params):
    ...
    for channel in tree.findall(sync_data("Y2hhbm5lbA==")):
        ...
        if kirjeldus:
           ...
           nyyd = sync_data("QUhPUkE6IA==") +nyyd[0] #EPG AHORA:
           jargmine = kirjeldus.partition(")\n")
           jargmine = jargmine[2].partition("(")
           jargmine = sync_data("U0lHVUlFTlRFOiA=") +jargmine[0] #EPG SIGUIENTE:
           kokku = nyyd+jargmine
        else:
           kokku = ""
        if pilt:
...
def grab_epg():
    ...
    req.add_header(sync_data("VXNlci1BZ2VudA==") , vod_channels("S29kaSBwbHVnaW4gYnkgU2VkeXVz"))
...
def execute_ainfo(params):
    ...
    if seis: # este if es para mostrar la cuenta como activa o inactiva
        seis = sync_data("QWN0aXZh")
    else:
        seis = sync_data("SW5hY3RpdmE=")
        
    if aegub:
       aegub = datetime.datetime.fromtimestamp(int(aegub)).strftime('%H:%M %d.%m.%Y')
    else:
       aegub = vod_channels("TnVuY2E=") 
    rabbits = kasutajaAndmed[vod_channels("aXNfdHJpYWw=")]
    if rabbits == "0":
       rabbits = sync_data("Tm8=")
    else:
       rabbits = sync_data("U2k=")
    #MENU MI CUENTA
    leavemealone = kasutajaAndmed[get_live("bWF4X2Nvbm5lY3Rpb25z")]
    polarbears = kasutajaAndmed[sync_data("dXNlcm5hbWU=")]
    plugintools.add_item( action="",   title=sync_data("W0NPTE9SID0gd2hpdGVdVXN1YXJpbzogWy9DT0xPUl0=")+polarbears , thumbnail="" , fanart=os.path.join(LOAD_LIVE,sync_data("YmFja2dyb3VuZC5wbmc=")) , folder=False )
    plugintools.add_item( action="",   title=sync_data("W0NPTE9SID0gd2hpdGVdRXN0YWRvOiBbL0NPTE9SXQ==")+seis , thumbnail="" , fanart=os.path.join(LOAD_LIVE,sync_data("YmFja2dyb3VuZC5wbmc=")) , folder=False )
    plugintools.add_item( action="",   title=get_live("W0NPTE9SID0gd2hpdGVdQ2FkdWNpZGFkOiBbL0NPTE9SXQ==")+aegub , thumbnail="" , fanart=os.path.join(LOAD_LIVE,sync_data("YmFja2dyb3VuZC5wbmc=")) , folder=False )
    plugintools.add_item( action="",   title=vod_channels("W0NPTE9SID0gd2hpdGVdQ3VlbnRhIGRlbW86IFsvQ09MT1Jd")+rabbits , thumbnail="" , fanart=os.path.join(LOAD_LIVE,sync_data("YmFja2dyb3VuZC5wbmc=")) , folder=False )
    plugintools.add_item( action="",   title=vod_channels("W0NPTE9SID0gd2hpdGVdQ29uZXhpb25lcyBtYXhpbWFzOiBbL0NPTE9SXQ==")+leavemealone , thumbnail="" , fanart=os.path.join(LOAD_LIVE,sync_data("YmFja2dyb3VuZC5wbmc=")) , folder=False )
    plugintools.set_view( plugintools.LIST )
